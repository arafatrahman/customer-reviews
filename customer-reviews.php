...
/**
 * Add column visibility options to the reviews admin screen, with save support.
 */
function ctrw_reviews_screen_options() {
    $screen = get_current_screen();
    if ($screen && $screen->id === 'toplevel_page_wp-review-plugin') {
        // Get saved user meta for column visibility
        ...
        ?>
        <style>
            .ctrw-col-toggle { margin-right: 10px; }
        </style>
        <div id="ctrw-column-visibility" style="margin: 1em 0;">
            <strong>Show/Hide Columns:</strong>
            <?php foreach ($defaults as $col => $def): ?>
                <label class="ctrw-col-toggle">
            ...
            <?php endforeach; ?>
            <button id="ctrw-save-cols" class="button button-secondary" style="margin-left:10px;">Save</button>
            <span id="ctrw-save-cols-msg" style="margin-left:10px;display:none;"></span>
        </div>
        <script>
        (function($){
            function ctrw_toggle_columns() {
                $('#ctrw-column-visibility .ctrw-toggle-col').each(function(){
            ...
            $(document).ready(function(){
                $('#ctrw-column-visibility .ctrw-toggle-col').on('change', ctrw_toggle_columns);
                ctrw_toggle_columns();

                $('#ctrw-save-cols').on('click', function(e){
                    e.preventDefault();
                    var data = {};
                    $('#ctrw-column-visibility .ctrw-toggle-col').each(function(){
                        data[$(this).data('col')] = $(this).is(':checked') ? 1 : 0;
                    });
                    $('#ctrw-save-cols').prop('disabled', true);
                    $('#ctrw-save-cols-msg').text('Saving...').show();
                    $.post(ajaxurl, {
                        action: 'ctrw_save_column_visibility',
                        columns: data,
                        _wpnonce: '<?php echo wp_create_nonce('ctrw_save_cols'); ?>'
                    }, function(resp){
                        $('#ctrw-save-cols').prop('disabled', false);
                        $('#ctrw-save-cols-msg').text('Saved!').fadeOut(2000);
                    });
                });
            });
        })(jQuery);
...
add_action('admin_notices', 'ctrw_reviews_screen_options');

/**
 * AJAX handler to save column visibility.
 */
